...
function camera.lookAt(x, y, z, xAt, yAt, zAt)
  ...
  fpsController.direction = math.pi / 2 - math.atan(dz, dx)
  fpsController.pitch = math.atan(dy, math.sqrt(dx ^ 2 + dz ^ 2))
...
function camera.updateOrthographicMatrix(size)
  camera.projectionMatrix:setOrthographicMatrix(camera.fov, size or 5, camera.nearClip, camera.farClip,
    camera.aspectRatio)
end
...
function camera.firstPersonLook(dx, dy)
  ...
  camera.lookInDirection(camera.position[1], camera.position[2], camera.position[3], fpsController.direction,
    fpsController.pitch)
end
